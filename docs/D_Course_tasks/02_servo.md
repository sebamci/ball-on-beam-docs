...
- The servo motor is connected to the designated `SERVO` Molex KK output on the Arduino shield:
    ...
    - **Black wire**: GND (B)
    - **White wire**: Signal (W)
- In Simulink, open your model and drag the `Servo_Position_Controller` block from the custom library into your diagram.
...
- Use a `Constant` block with a value of `0` connected to the `Servo_Position_Controller` block.
...
> ⚠️ The input of the `Servo_Position_Controller` block directly represents the desired beam angle in **radians**.  
...
- Test the full motion range of the beam (±4° input ≈ ±0.07 rad).
- Ensure:
    - The servo moves freely.
    - No mechanical obstructions are present.
    - The beam is balanced and doesn't collide with the frame.
...
| Simulink Block   | Use `Servo_Position_Controller` from custom library       |
